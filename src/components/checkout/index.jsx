...
class checkOut extends Component {
    ...
    StripeClick = () => {
        ...
        let config = {
            headers: {
              Authorization: `Bearer ${this.props.customerInfo.token}`
            }
          };
        
        this.props.cartItems.map((product, index)=> 
        {
            productIds[index]={product_id:null}
            productIds[index].product_id=product.product_id})

        if (this.validator.allValid()) {
           axios.post('http://localhost:3000/order', {
               order:{customerId:this.props.customerInfo.customer_id,
               totalPrice:this.props.total},
               orderDetail:productIds},config

           ).then((result)=>{
               alert('order has been successfully received')
               this.setState({redirect:true})
               this.props.cartItems.map((items)=>{
                   this.props.removeFromCart(items)
    ...
    render (){
        ...
        return (
            <div>
                ...
                {/*SEO Support*/}
                <Helmet>
                    <title>Shopmate | CheckOut Page</title>
                </Helmet>
                {/*SEO Support End */}
...
const mapStateToProps = (state) => ({
    ...
    customerInfo:state.cartList.loginToken,
